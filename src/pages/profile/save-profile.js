...
export default async function saveProfile(data) {
  ...
  try {
    if (
      ...
      await Post("/cities", data["address"], token).then(result => {
        data["address"].id_city = result.data[0].id;
      });
      await Post("/neighborhoods", data["address"], token).then(result => {
        data["address"].id_neighborhood = result.data[0].id;
      });
      await Post("/states", data["address"], token).then(result => {
      ...
      if (data["user"].id_address) {
        ...
        await Post("/addresses", data["address"], token)
          .then(result => {
    ...
    await Put("/users/true/", data["user"], token)
      .then(() => {
